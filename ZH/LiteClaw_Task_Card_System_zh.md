...
> **使用原则：每张卡 = Claude Code 一次独立会话。完成后验收，再开下一张。** **绝不要一次丢多张卡。Agent 单次最优，持续附重必然熵增。**
...
```bash
# 每次只给 Claude Code 一张任务卡
claude -p "$(cat tasks/TASK-01.md)"

# 完成后，人工验收
# 验收通过 → 给下一张卡
# 验收失败 → 新会话重做这张卡（不要在同一会话里修修补补）
```
...
### TASK-01: Credential Store（密钥保险箱）
...
### TASK-03: Agent Firewall（Agent 权限防火墙）
...
### TASK-16: 审计日志生成器（Obsidian Markdown）
...
### TASK-17: Shell/终端引擎 + Claude Code 控制器
...
## 总计：29 张任务卡（TASK-00 ~ TASK-28）

每张卡 = Claude Code 一次会话 每张卡 = 30-90 分钟完成 每张卡 = 有明确的验收标准

**预计总工时：29 × 1小时 ≈ 29 小时 ≈ 5-6 个工作日（集中开发）**
