...
> **사용 원칙: 각 카드 = Claude Code 한 번의 독립 세션. 완료 후 검수하고, 다음 카드를 시작합니다.** **절대 여러 카드를 한꺼번에 넣지 마세요. Agent 단발이 최적이며, 지속적 부하는 반드시 엔트로피를 증가시킵니다.**
...
```bash
# 매번 Claude Code에 태스크 카드 한 장만 제공
claude -p "$(cat tasks/TASK-01.md)"

# 완료 후 수동 검수
# 검수 통과 → 다음 카드 제공
# 검수 실패 → 새 세션에서 이 카드 다시 수행 (같은 세션에서 수정하지 마세요)
```
...
### TASK-03: Agent Firewall (Agent 권한 방화벽)
...
### TASK-04: 보안 레이어를 Agent Loop에 통합
...
### TASK-07: Rate Limiter (로컬 속도 제한기)
...
### TASK-09: 라우터 레이어를 Agent Loop에 통합
...
### TASK-13: 컨텍스트 레이어 통합, 기존 context.py 대체
...
### TASK-16: 감사 로그 생성기 (Obsidian Markdown)
...
### TASK-17: Shell/터미널 엔진 + Claude Code 컨트롤러
...
### TASK-24: Gradio 기본 프레임워크 + Chat 인터페이스
...
## 합계: 29장의 태스크 카드 (TASK-00 ~ TASK-28)

각 카드 = Claude Code 한 번의 세션 · 각 카드 = 30-90분 완료 · 각 카드 = 명확한 수락 기준

**예상 총 소요시간: 29 × 1시간 ≈ 29시간 ≈ 5-6 근무일 (집중 개발)**
